...
#define LOG(rank, ...) printf("LOG%i: ", rank); printf(__VA_ARGS__); printf("\n");
...
double* dotProd(double**w,int r,int c, double *x,int n)
{
	...
	double* outputTemp= (double*)malloc((numProcs) * sizeof(double));
	double sent=0;
    ...
    MPI_Gather(&sent,1,MPI_DOUBLE,outputTemp,1,MPI_DOUBLE,numProcs-1,MPI_COMM_WORLD);		
	for(int i=0;i<c;i++)
...
double** dotProd2(double*x1,int rx1, double *x2,int cx2)
{
    ...
    double * sent=(double*)malloc(cx2 * sizeof(double));
	double * rec=(double*)malloc(cx2*numProcs * sizeof(double));
    ...
    if(myrank!=numProcs-1)
	{
		if(myrank<rx1)
		{
			for(int i = 0;i<cx2;i++)
			{
				sent[i]=x1[myrank]*x2[i];
			}
		}
	}
    MPI_Gather(sent, cx2, MPI_DOUBLE, rec, cx2, MPI_DOUBLE, numProcs-1, MPI_COMM_WORLD);
    if(myrank==numProcs-1)
    {
		int y=0;
        for (int i = 0; i < rx1; i++)
